...
最新主对照见[差距审计](browser-use-gap-audit.md)：按用户要求，以 Relay 自身接口和 Codex
Browser Use 原生 AX/截图/动作工作流比较复杂任务。本文保留此前的表单执行微基准和辅助路径数据，
不把内部 Playwright 分支作为整体 Browser Use 的代替。
...
Skill 入口最初由 459 行缩减到 135 行；本轮补充已验证的行为后为 141 行，详细 SDK/HTTP 内容按需加载。执行提示词强调：
...
| 文档按需加载，避免每轮塞全部 API | 简短入口与按需引用文件 | 行数下降不是 token 成本或任务完成时间的实测结论 |
